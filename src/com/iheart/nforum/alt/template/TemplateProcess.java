		...
		if(updateCurStatus1 > 0) {
			...
			if (updateCurStatus2 > 0) {
				...
				if ("200".equals(code)) {
					...
					// 등록 성공 결과처리...
					int succDataCnt = dao.updateResponseTemplateData(regRes);
					if(succDataCnt > 0) {
						// 모두 성공
						...
						logger.info("TEMPLATE_CODE:" + dataVO.getTemplateCode() + "//ACTION:REG-RES//CODE://" + code + "DESC:Update Template Response Data Fail");
					}
				}
				else {
					// CUR_STATUS = 2, INSPECTION_STATUS = NULL
				...
				logger.info("TEMPLATE_CODE:" + dataVO.getTemplateCode() + "//ACTION:REG-RES//DESC:CUR_STATUS [1 -> 2] UPDATE FAIL");
			}
			
		} 
		else {
			logger.info("TEMPLATE_CODE:" + dataVO.getTemplateCode() + "//ACTION:REG-REQ//DESC:CUR_STATUS [0 -> 1] UPDATE FAIL");
		}
